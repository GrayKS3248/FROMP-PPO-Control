...
class FES():
    ...
    def step(self, action):
        ...
        next_magnitude = np.clip(self.input_magnitude + np.clip(action[1], -self.max_magnitude_rate, self.max_magnitude_rate), 0.0, self.peak_thermal_rate)
        ...
        state = np.concatenate((self.temperature_grid[0::10], [self.temperature_grid[-1]], 
                                temperature_grid_rate[0::10], [temperature_grid_rate[-1]], 
                                [self.front_position], [self.front_rate], 
                                [self.input_location], [np.clip(action[0], -self.max_movement_rate, self.max_movement_rate)],
                                [np.clip(action[1], -self.max_magnitude_rate, self.max_magnitude_rate)]))
    ...
    def reset(self):
        ...
        state = np.concatenate((self.temperature_grid[0::10], [self.temperature_grid[-1]], 
                                self.temperature_grid[0::10]*0.0, [self.temperature_grid[-1]*0.0], 
                                [self.front_position], [self.front_rate], 
                                [self.input_location], [0.0],
                                [0.0]))
        return state