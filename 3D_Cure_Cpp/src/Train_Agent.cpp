...
*/
int load_config(string& autoencoder_path_str, string& estimator_path_str, int& total_trajectories, int& steps_per_trajectory, int& trajectories_per_batch, int& epochs_per_batch, bool& reset_std, bool&input_rates, double& gamma, double& lamb, double& epsilon, double& start_alpha, double& end_alpha, double& frame_rate)
{
...
* @return 0 on success, 1 on failure
*/
int run(Finite_Difference_Solver* FDS, PyObject* agent, PyObject* error_estimator, PyObject* save_render_plot, int total_trajectories, int steps_per_agent_cycle, int steps_per_frame, int steps_per_trajectory, auto &start_time, string& configs_string, bool input_rates)
{
	...
	for (int i = 0; i < total_trajectories; i++)
	{
		...
		{
			image_sequence.push_back(FDS->get_coarse_temp_z0(true));
		}
		...
		while (!done)
		{
			...
			if (run_agent)
			{
				...
				done = FDS->step(action_1, action_2, input_rates, action_3);
		...
		if (input_rates)
		{
			x_loc_stdev.push_back(0.5*FDS->get_max_input_slew_speed()*stdev_1);
			y_loc_stdev.push_back(0.5*FDS->get_max_input_slew_speed()*stdev_2);		
		}
		else
		{
			x_loc_stdev.push_back(0.5*FDS->get_coarse_x_len()*stdev_1);
			y_loc_stdev.push_back(0.5*FDS->get_coarse_y_len()*stdev_2);		
		}
		mag_rate_stdev.push_back(FDS->get_peak_input_mag()*stdev_3);
	...
	if(store_options(save_render_plot, FDS->get_control_mode(), input_rates, configs_string) == 1) {return 1;}
...
{	
	...
	double end_alpha;
	double frame_rate;
	if (load_config(autoencoder_path_str, estimator_path_str, total_trajectories, steps_per_trajectory, trajectories_per_batch, epochs_per_batch, reset_std, input_rates, gamma, lamb, epsilon, start_alpha, end_alpha, frame_rate) == 1) 
	{ 
		cin.get();
		return 1; 
	}
	...
	if (run(FDS, agent, error_estimator, save_render_plot, total_trajectories, steps_per_agent_cycle, steps_per_frame, steps_per_trajectory, start_time, configs_string, input_rates) == 1)
	{ 
		Py_FinalizeEx();
		cin.get();
		return 1; 
