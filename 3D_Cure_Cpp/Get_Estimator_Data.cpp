...
int run(Finite_Difference_Solver* FDS, int total_trajectories, int num_states, int steps_per_control_cycle, int steps_per_state_frame, int tot_num_sim_steps, int samples_per_trajectory, int samples_per_batch, int target_code, string path)
{
	for (int curr_batch = 0; curr_batch < (int)floor(((double)total_trajectories*(double)samples_per_trajectory)/(double)samples_per_batch); curr_batch++)
	{
		...
		for (int curr_traj = 0; curr_traj < (int)floor((double)samples_per_batch/(double)samples_per_trajectory); curr_traj++)
		{
			...
			// User readout
			print_training_info(curr_traj+curr_batch*(int)floor((double)samples_per_batch/(double)samples_per_trajectory), total_trajectories);
			...
			// Final user readout
			if (curr_traj+curr_batch*(int)floor((double)samples_per_batch/(double)samples_per_trajectory) == total_trajectories - 1) { print_training_info(curr_traj+curr_batch*(int)floor((double)samples_per_batch/(double)samples_per_trajectory), total_trajectories); }
		}
...
{	
	...
	int target_code;
	string path;
	if (load_config(total_trajectories, samples_per_trajectory, samples_per_batch, actions_per_trajectory, num_states, time_between_state_frame, target_code, path) == 1) { return 1; }
	...
	if (run(FDS, total_trajectories, num_states, steps_per_control_cycle, steps_per_state_frame, tot_num_sim_steps, samples_per_trajectory, samples_per_batch, target_code, path) == 1) { return 1; }
