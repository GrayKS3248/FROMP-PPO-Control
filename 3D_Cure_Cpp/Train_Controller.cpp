...
*/
void print_params(int encoder_output_size, int samples_per_trajectory, int samples_per_batch, double ae_start_alpha, double ae_end_alpha, int x_dim, int y_dim, int steps_per_trajectory, int trajectories_per_batch, int num_epochs, double gamma, double lamb, double epsilon, double rl_start_alpha, double rl_end_alpha)
{
...
*/
void save_autoencoder_training_data(vector<double> MSE_loss, PyObject* autoencoder)
{
...
*/
void save_agent_training_data(PyObject* trained_agent, vector<double> r_per_episode, vector<double> x_rate_stdev, vector<double> y_rate_stdev, vector<double> mag_stdev, vector<double> value_error,
...
* @param The x dimension of the autoencoder input
* @param The bottleneck dimension of the autoencoder
* @return Tuple containing the training data, the updated finite element solver, and the trained PPO agent
*/
auto run(Finite_Element_Solver FES, PyObject* agent, int total_trajectories, int steps_per_agent_cycle, int steps_per_frame, int steps_per_trajectory, PyObject* autoencoder, int tot_num_sim_steps, int samples_per_trajectory, int x_dim, int encoder_output_size)
{
	...
	for (int i = 0; i < total_trajectories; i++)
	{
		...
		while (!done)
		{
			...
			if (run_agent)
			{
				...
				{
					PyErr_Print();
					fprintf(stderr, "Failed to update agent\n");
					cin.get();
				}
		...
		{
			print_training_info(total_trajectories-1, total_trajectories, prev_episode_reward, steps_per_trajectory, r_per_episode, best_episode, MSE_loss);
		}
	}
	...
	{
		PyErr_Print();
		fprintf(stderr, "Failed to gather value estimation error training data\n");
		cin.get();
	}
	...
	auto out = make_tuple(r_per_episode, x_rate_stdev, y_rate_stdev, mag_stdev, value_error, best_input_location_x,
	best_input_location_y, best_input_percent, best_sim_time, best_target, best_temperature_field, best_cure_field,
	best_front_location, best_front_velocity, best_front_temperature, best_episode, MSE_loss);
	return out;
}
...
{	
	...
	// Init agent
	Py_Initialize();
	PyObject* agent = init_agent(FES.get_num_state(encoder_output_size), steps_per_trajectory, trajectories_per_batch, minibatch_size, num_epochs, gamma, lamb, epsilon, rl_start_alpha, rl_decay_rate, load_agent, reset_stdev);
	if (agent == NULL)
	{
		PyErr_Print();
	...
	// Init autoencoder
	PyObject* autoencoder = init_autoencoder(ae_start_alpha, ae_decay_rate, x_dim, y_dim, encoder_output_size, samples_per_batch, load_autoencoder);
	if (autoencoder == NULL)
	{
		PyErr_Print();
	...
	print_params(encoder_output_size, samples_per_trajectory, samples_per_batch, ae_start_alpha, ae_end_alpha, x_dim, y_dim, steps_per_trajectory, trajectories_per_batch, num_epochs, gamma, lamb, epsilon, rl_start_alpha, rl_end_alpha);
	FES.print_params();

	// Train agent
	cout << "\nTraining agent..." << endl;
	auto start_time = chrono::high_resolution_clock::now();
	auto [r_per_episode, x_rate_stdev, y_rate_stdev, mag_stdev, value_error, best_input_location_x,
	best_input_location_y, best_input_percent, best_sim_time, best_target, best_temperature_field, 
	best_cure_field,best_front_location, best_front_velocity, best_front_temperature, best_episode, 
	MSE_loss] = run(FES, agent, total_trajectories, steps_per_agent_cycle, steps_per_frame, steps_per_trajectory, 
	autoencoder, tot_num_sim_steps, samples_per_trajectory, x_dim, encoder_output_size);
	...
	save_autoencoder_training_data(MSE_loss, autoencoder);
	
	// Save agent and agent training data
	save_agent_training_data(agent, r_per_episode, x_rate_stdev, y_rate_stdev, mag_stdev, value_error,
