...
class Autoencoder:
    ...
    # @param Verbose
    def __init__(self, alpha, decay, load_path="", dim_1=0, dim_2=0, norm_min=0.0, norm_max=1.0, num_targets=0, num_latent=0, noise_stdev=0.0, occ_prob=0.0, occ_max=1, occ_min_area=0.0, occ_max_area=0.0, verbose=True):
    ...
    def learn(self, input_batch, targets_batch, take_snapshot=False):
        ...
        for i in range(batch_length):
            ...
            # Convert to tensor of correct dimensions
            noisy_occluded_norm_sq_input_image = torch.tensor(noisy_occluded_norm_sq_input_image, requires_grad=True)
            noisy_occluded_norm_sq_input_image = noisy_occluded_norm_sq_input_image.reshape(1,1,noisy_occluded_norm_sq_input_image.shape[0],noisy_occluded_norm_sq_input_image.shape[1]).float()
            noisy_occluded_norm_sq_input_image = noisy_occluded_norm_sq_input_image.to(self.device)
            
            # Forward through model
            if self.num_latent >= 1:
                norm_sq_output_images, output_latent = self.model.forward(noisy_occluded_norm_sq_input_image)
            else:
                norm_sq_output_images = self.model.forward(noisy_occluded_norm_sq_input_image)
