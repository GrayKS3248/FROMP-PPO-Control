...
if __name__ == "__main__":
    ...
    print("Calculate L1 matrix...")
    L1 = sparse.csc_matrix(sparse.eye(len(verts))+(kappa*dt/(2.0*rho*cp))*laplacian)
    tol =0.0001*np.min(abs(L1[L1.nonzero()]))
    
    print("Calculate L2 matrix...")
    lu_obj = sparse.linalg.splu(sparse.csc_matrix(sparse.eye(len(verts))-(kappa*dt/(2.0*rho*cp))*laplacian))
    b = np.zeros((len(verts),1))
    dat = []
    row_inds = []
    col_inds = []
    for i in range(len(verts)):
        ...
        col = lu_obj.solve(b)
        col[abs(col)<tol]=0.0
        dat.append(col[np.nonzero(col)])
        row_inds.append(np.nonzero(col)[0])
        col_inds.append(np.ones(np.nonzero(col)[0].shape,np.int64)*i)
    dat = np.concatenate(dat,axis=0)
    row_inds = np.concatenate(row_inds,axis=0)
    col_inds = np.concatenate(col_inds,axis=0)
    L2 = sparse.csc_array((dat,[row_inds,col_inds])).toarray()
